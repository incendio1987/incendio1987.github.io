...
function HomeV2() {
  ...
  const [lightbox, setLightbox] = React.useState(null); // project obj or null
  React.useEffect(() => {
  ...
  const css = `
    ...
    .h2-lightbox .lb-close:hover { background: var(--ink); color: var(--paper); }
    .h2-lightbox .lb-card {
      position: relative;
    ...
    .h2-lightbox .lb-meta .t { font-size: 18px; letter-spacing: -0.02em; font-weight: 900; }
    .h2-lightbox .lb-meta .c { font-size: 9px; letter-spacing: 0.3em; opacity: 0.6; font-weight: 700; }
  `;
  ...
  return (
    <div className="h2">
      ...
      <section className="h2-bonitas">
        ...
        <div className="h2-carousel-wrap"
          ...
          <div className="h2-carousel" ref={carRef}>
            {featured.map((p, i) => (
              <a className="h2-cb" key={p.id} onClick={() => setLightbox(p)} style={{cursor:"pointer"}}>
      ...
      {/* ────── LIGHTBOX ────── */}
      {lightbox && (
        <div className="h2-lightbox" onClick={() => setLightbox(null)}>
          <button className="lb-close" onClick={() => setLightbox(null)}>✕</button>
          <div className="lb-card" onClick={e => e.stopPropagation()}>
            <div className="lb-img">
              <img src={lightbox.cover} alt={lightbox.title} />
            </div>
            <div className="lb-meta">
              <span className="t">{lightbox.title}</span>
              <span className="c">{lightbox.cat} · '{lightbox.year?.slice(-2)}</span>
            </div>
          </div>
        </div>
      )}
